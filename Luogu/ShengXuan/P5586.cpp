...
//	int rebuildfrac = std::max(100,m / 10);
	...
	for(int i = 1,opt,l1,r1,l2,r2,k;i<=m;++i)
	{
		...
		if(opt == 1) printf("%d\n",last = asksum(l1,r1));
		else if(opt == 2) read(k),k ^= last,seqset(l1,r1,k);
		else if(opt == 3) read(k),k ^= last,seqadd(l1,r1,k);
		else if(opt == 4) read(l2),read(r2),l2 ^= last,r2 ^= last,seqcopy(l1,r1,l2,r2);
		else if(opt == 5) read(l2),read(r2),l2 ^= last,r2 ^= last,seqswap(l1,r1,l2,r2);
