...
void change(int x)
{
    if (c[x]) 
    {
        set<int>::iterator it = S[top[x]].find(dfn[x]);
        if(it != S[top[x]].end()) S[top[x]].erase(it);
    }
    else S[top[x]].insert(dfn[x]);
    c[x] ^= 1;
}
int ask(int x)
{
    ...
    while(x)
    {
        if(S[top[x]].size() && *S[top[x]].begin() <= dfn[x]) ans = *S[top[x]].begin();
        x = fa[top[x]];
    }
    return ans > 0 ? id[ans] : -1;
}
int main()
{
    read(n), read(m);
